...
\\end{{document}}"""
    ...
    st.code(legend_code, language="latex")
        # --- START OF AI IMPORTER ---
    st.markdown("---")
    st.subheader("🤖 AI-Snippet Importer")
    st.info("Paste TikZ code generated by AI (ChatGPT/Claude) below. We will wrap it in a publication-ready preamble.")
    
    # 1. Input area for raw AI code
    ai_raw_code = st.text_area("Paste Raw TikZ Code here:", placeholder=r"\begin{tikzpicture} ... \end{tikzpicture}", height=200, key="ai_importer_area")
    
    if ai_raw_code:
        # 2. Cleaning Logic: Remove common AI bloat like document headers if present
        ...
        # 3. Wrapping in your custom biological preamble
        # Ensure build_full_tikz_document is defined in your helper functions
